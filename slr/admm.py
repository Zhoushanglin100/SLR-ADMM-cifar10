...
def test_sparsity(args, ADMM, model):
    ...
    if args.sparsity_type == "irregular":
        ...
        for i, (name, W) in enumerate(model.named_parameters()):
            ...
            total_nonzeros += np.sum(W != 0)
            print(
                "channel sparsity of layer {} is {}".format(
...
def z_u_update(
    args, ADMM, model, device, train_loader, optimizer, epoch, data, batch_idx, writers
):
...
def combined_masked_retrain(args, ADMM, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % args.print_freq == 0:
            ...
            print(
                "({}) ({}) cross_entropy loss: {}".format(
                    args.sparsity_type, args.optimizer, loss
                )
            )
            print(
