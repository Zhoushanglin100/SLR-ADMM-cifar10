...
def test_irregular_sparsity(model):
    ...
    for name, weight in model.named_parameters():
        ...
        print(
            "irregular zeros: {}, irregular sparsity is: {:.4f}".format(
                zeros, zeros / (zeros + non_zero)
            )
        )

    print("---------------------------------------------------------------------------")
    print(
        "total number of zeros: {}, non-zeros: {}, zero sparsity is: {:.4f}".format(
            total_zeros, total_nonzeros, total_zeros / (total_zeros + total_nonzeros)
        )
    )
    print(
        "only consider conv layers, compression rate is: {:.4f}".format(
...
def test_column_sparsity(model):
    ...
    for name, weight in model.named_parameters():
        if len(weight.size()) == 4:  # only consider conv layers
            ...
            print(
                "empty column of {} is: {}. column sparsity is: {:.4f}".format(
    ...
    print(
        "total number of column: {}, empty-column: {}, column sparsity is: {:.4f}".format(
    ...
    print(
        "only consider conv layers, including unused weight, compression rate is: {:.4f}".format(
...
def calculate_unused_weight(model):
        ...
        if len(weight.size()) == 4:
            ...
            if m != 1:
                ...
                for i in range(weight3d.size()[1]):
                    non_zero_filter = np.where(
                        weight3d[:, i, :].cpu().detach().numpy().any(axis=1)
                    )[0]
                ...
                for filter_index in empty_channel_index:
                    ...
                    filter_unused_num += np.sum(
                        target_filter.cpu().detach().numpy() != 0
                    )  # != 0 to calculate sparsity
            ...
            if n != counter:
                ...
                for channel_index in empty_filter_index:
                    ...
                    channel_unused_num += np.sum(
                        target_channel.cpu().detach().numpy() != 0
                    )  # != 0 to calculate sparsity
    ...
    print(
        "total unused weight number (column => prev filter / filter => next column): ",
        total_unused_number,
    )
...
def test_chanel_sparsity(model):
    ...
    for name, weight in model.named_parameters():
        if len(weight.size()) == 4:
            ...
            print(
                "({} {}) almost empty channel: {}, total channel: {}. ratio: {}%".format(
                    name,
                    weight.size(),
                    almost_empty_channel,
                    weight2d.size()[1],
                    100.0 * almost_empty_channel / weight2d.size()[1],
                )
            )
...
def test_filter_sparsity(model):
        ...
        if len(weight.size()) == 4:  # only consider conv layers
            ...
            for i in range(filter_num):
                if np.sum(weight[i, :, :, :].cpu().detach().numpy()) == 0:
                    empty_filters += 1
            print(
                "empty filter of {} is: {}. filter sparsity is: {:.4f}".format(
    ...
    print(
        "total number of filters: {}, empty-filters: {}, filter sparsity is: {:.4f}".format(
