...
try:
    ...
    has_wandb = True
    wandb.init(project="RT3", entity="zhoushanglin100")
except ImportError:
    has_wandb = False
...
def main(args):
        ...
        for i in range(args.rho_num):
            ...
            if i == 0:
                model_path = os.path.join(
                    args.save_dir, "baseline", args.baseline_model
                )
                print("Path is:{}".format(model_path))

                model.load_state_dict(
                    torch.load(model_path)
                )  # admm train need basline model
                model.cuda()
                ...
                model.load_state_dict(
                    torch.load(
                        "model_prunned/mnist_{}_{}_{}.pt".format(
                            current_rho / 10, args.config_file, args.sparsity_type
                        )
                    )
                )
                model.cuda()
            ADMM = admm.ADMM(
                args, model, "profile/" + args.config_file + ".yaml", rho=current_rho
            )
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                if args.combine_progressive:
                    admm_loss, mixed_loss, loss = admm.admm_masked_train(
                        args, ADMM, model, device, train_loader, optimizer, epoch
                    )
                else:
                    mixed_loss, loss = train(
                        args,
                        ADMM,
                        model,
                        device,
                ...
                if (best_prec1 < prec1) and (epoch != 1):
                    ...
                    ## remove old model
                    old_file = "cifar10_{}_{}_{}_{}{}.pt".format(
                        args.arch,
                        best_prec1,
                        args.config_file,
                        args.sparsity_type,
                    ...
                    torch.save(
                        ...
                        f"{args.save_dir}/{args.arch}/{args.optimization}"
                ...
                if args.optimization == "savlr":
                    ...
                    if not has_wandb:
                        ...
                        condition_d["Condition2"] = (
                            condition_d.get("Condition2", []) + ADMM.condition2
                        )
            ...
            if not os.path.exists(hp_dir):
                os.makedirs(hp_dir)
            torch.save(
                model_forhard.state_dict(),
                hp_dir
                + "cifar10_{}_{}_{}_{}{}.pt".format(
                    args.arch, prec_hp, args.config_file, args.sparsity_type, args.ext
                ),
            )
            ...
            if not has_wandb:
                ...
                f = open(
                    args.save_dir + "/results/mixed_losses{}.pkl".format(current_rho),
                    "wb",
                )
                ...
                f = open(
                    args.save_dir + "/results/ce_loss{}.pkl".format(current_rho), "wb"
                )
    ...
    if args.masked_retrain:
        ...
        for epoch in range(1, args.retrain_epoch + 1):
            ...
            epoch_loss = []
            scheduler = optim.lr_scheduler.CosineAnnealingLR(
                optimizer, T_max=epoch * len(train_loader), eta_min=4e-08
            )
            scheduler.step()
            ...
            if args.combine_progressive:
                idx_loss_dict = admm.combined_masked_retrain(
                    args, ADMM, model, device, train_loader, optimizer, epoch
                )
            else:
                idx_loss_dict = admm.masked_retrain(
                    args, ADMM, model, device, train_loader, optimizer, epoch
                )
